...
minetest.register_node("online_shop:shop_server", {
			  ...
			  -- Called after the node is dug using minetest.node_dig or minetest.dig_node
			  after_dig_node = function(pos, oldnode, oldmetadata, digger)
			     local pos_string = minetest.pos_to_string(pos)
			     print(dump(oldmetadata))
			     local store_name = oldmetadata:get_string("store_name")
			     -- The store may not have been initialized before it was destroyed
			     if store_name ~= nil and store_name ~= "" then
				online_shop.remove_from_shop(store_name, pos_string)
			     end
...
function online_shop.register_shop(shop_def)
   ...
   if storage:get_string("stores_list") == "" then
      set_val("stores_list", {})
   end
   
   local stores_dict = get_val("stores_list")

   -- If it already exists in the table, we don't care
   -- Just never have two of the same position

   stores_dict[minetest.hash_node_position(shop_def.pos)] = shop_def
...
function online_shop.unregister_shop(shop_def)
   ...
   if shop_def.shop_name == nil or shop_def.item_label == nil then
      return
   end
   
   local stores_dict = get_val("stores_dict")
   stores_dict[shop_def.shop_name] = nil
